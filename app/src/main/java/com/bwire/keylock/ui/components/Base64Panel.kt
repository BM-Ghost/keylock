...
import com.bwire.keylock.domain.crypto.Base64Engine
 ...
 * 
 * Features:
 * - Encode: ASCII/Hexadecimal -> Base64
 * - Decode: Base64 -> ASCII
 */
@Composable
fun Base64Panel(
...
private fun getPlaceholder(tab: Int, encoding: DataEncoding): String {
    return if (tab == 0) {
        ...
        when (encoding) {
            ...
            DataEncoding.HEXADECIMAL -> "Enter hexadecimal data (e.g., 48656C6C6F)"
        }
    } else {
        // Decode
        "Enter Base64 encoded data (e.g., SGVsbG8gV29ybGQ=)"
    }
}
...
private fun encodeOperation(
    ...
    val result = Base64Engine.encode(data, encoding)
    ...
    result.onSuccess { output ->
        ...
        val message = buildString {
            ...
            appendLine("Base64: Encoding finished")
            appendLine("****************************************")
...
private fun decodeOperation(
    ...
    val result = Base64Engine.decode(data)
    ...
    result.onSuccess { output ->
        ...
        val message = buildString {
            appendLine("[$timestamp]")
            appendLine("Base64: Decoding finished")
            appendLine("****************************************")
